...
const hasChildrenValues = (arr, character) => {
  let result = false;
  Object.values(arr).forEach(item => item.name === character && item.children.length > 0 ? result = true : '');
  return result;
};
...
const hasChildrenEntries = (arr, character) => {
  let result = false;
  arr.forEach(item => item.name === character && item.children.length > 0 ? result = true : '');
  return result;
};
...
const houseSize = (arr) => {
  const sizes = [];
  arr.forEach(item => sizes.push({ house: item.house, members: (1 + (item.spouse ? 1 : 0) + item.children.length) }));
  return sizes;
};
...
const houseSurvivors = (arr) => {
  const survivors = [];
  arr.forEach(item => survivors.push(
    {
      house: item.house,
      members: (1 + (item.spouse && !deceasedSpouses.includes(item.spouse) ? 1 : 0) + item.children.length)
    })
  );
  return survivors;
};
