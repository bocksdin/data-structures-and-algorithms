...
const count = (target, input) => {
  return input.reduce((acc, arr) => {
    return acc += arr.filter(value => value === target).length;
  }, 0);
};
...
const totalSum = (input) => input.reduce((acc, arr) => acc += arr.reduce((total, value) => total += value), 0);
...
const divisibleByFiveTwoToThePower = (input) => {
  let result = [];
  input.map(arr => {
    const filteredArr = arr.filter(value => typeof value === 'number' && value % 5 === 0);
    result.push(filteredArr.map(value => Math.pow(2, value)));
  });
  return result;
};
...
let findMaleAndFemale = (data) => {
  let result = [];
  const filteredData = data.filter(obj => obj.gender === 'male' || obj.gender === 'female');
  result = filteredData.map(obj => obj.name);
