...
const standardizePhoneNumbers = (arr) => arr.map(num => num.split('').filter(char => char.charCodeAt(0) > 47 && char.charCodeAt(0) < 58).join(''));
...
const onlyOddChars = (str) => str.split('').filter((char, idx) => idx % 2 === 1 ? char : '').join('');
