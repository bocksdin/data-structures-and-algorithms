...
const totalSumCSV = (str) => str.split(',').reduce((acc, num) => { return acc += parseInt(num); }, 0);
...
const extractVowels = (str) => {
  ...
  result.push(str.split('').filter(char => /[aeiou]/i.test(char)).sort().join(''));
  return result;
};
